...
class DiceGameViewModel(private val context: Context, private val gold: Int) : ViewModel() {
	// Состояние игры
	var playerDiceValue by mutableIntStateOf(1)
		private set
	var botDiceValue by mutableIntStateOf(1)
		private set
	var gameState by mutableStateOf(GameState.BETTING)
		private set

	// Ставка и золото
	var playerGold by mutableIntStateOf(gold) // Начальное золото игрока
		private set
	var currentBet by mutableIntStateOf(10) // Текущая ставка
		private set
	var maxBet by mutableIntStateOf(gold) // Максимальная ставка
		private set
	...
	fun placeBet() {
		if (currentBet in 10..playerGold) {
	...
	companion object {
		fun createFactory(initialGold: Int): ViewModelProvider.Factory = viewModelFactory {
			initializer {
				val context = this[ViewModelProvider.AndroidViewModelFactory.APPLICATION_KEY] as Context
				DiceGameViewModel(context, initialGold)
			}
		}
